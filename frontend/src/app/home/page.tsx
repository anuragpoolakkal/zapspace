...
export default function Home() {
    ...
    return <div className="flex w-screen h-screen bg-white flex-col text-black p-5">
        ...
        {/* Onboarding */}
        {showOnboarding || showLogin || showSignUp ? "" : <div style={{ position: "relative", background: "black", overflow: "hidden"}} className="rounded-2xl hero h-[90vh] w-full">
            <img className="absolute object-cover w-full h-full" style={{zIndex: "0", filter: "opacity(0.5)"}} src={businessData?.name ? `${businessCategories[businessData?.category].bgImage}")` : "https://images.unsplash.com/photo-1576500714954-8a687d0ea1ef?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1170&q=80"}/>
            <div className="hero-content flex-col lg:flex-row">
                ...
                <div>
                    <h1 className="max-w-[70%] text-5xl text-white font-bold">{businessData.name ? `Welcome ${businessData.name}!` : "Put your business on the Internet with ease, in just a few clicks!"}</h1>
                    ...
                    }}>{businessData.name ? "Dashboard" : "GET STARTED"}<FiArrowRight/></button>
                </div>
            </div>
        </div>}
